...
const select = document.getElementById("pet");
    ...
    dados.pets.forEach(pet => {
        ...
        option.textContent = pet.nome; // Define o texto exibido da opção
...
function criarReserva(event) {
    ...
    if (new Date(checkin) >= new Date(checkout)) {
        mostrarMensagemErro("A data de partida deve ser maior que a data de chegada.");
        return;
    }
    ...
    const dados = JSON.parse(localStorage.getItem("hotelPetDados")) || { reservas: [] };
