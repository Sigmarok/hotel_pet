...
function carregarReserva(id) {
    ...
    if (reserva) {
        ...
        const totalDiarias = calcularTotalDiarias(reserva.chegada, reserva.partida, 40);
        document.getElementById("total-diarias").innerText = totalDiarias.toFixed(2);
    } else {
...
function salvarAlteracoes(id) {
    ...
    if (reservaIndex !== -1) {
        ...
        dados.reservas[reservaIndex].status = document.getElementById("status").value;
        dados.reservas[reservaIndex].chegada = document.getElementById("checkin").value;
        dados.reservas[reservaIndex].partida = document.getElementById("checkout").value;
        dados.reservas[reservaIndex].notas = document.getElementById("notas").value;
        dados.reservas[reservaIndex].anotacoes = document.getElementById("anotacoes").value;
